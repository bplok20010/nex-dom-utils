import { push, each, merge, lowercase, isDefined, isFunction, isString, makeArray, isWindow, isUndefined, isObject, NODE_TYPE_DOCUMENT, NODE_TYPE_ATTRIBUTE, NODE_TYPE_TEXT, NODE_TYPE_COMMENT, NODE_TYPE_DOCUMENT_FRAGMENT, NODE_TYPE_ELEMENT, extend } from './selector/utils';
...
import { hasClass, addClass, removeClass, toggleClass } from './classes';
...
function jqLiteWidthOrHeightCreator(type) {
	...
	return function (elem, value) {
		...
		if (value === undefined) {
			...
			const borderOrPadding = parseFloat(css(elem, `border${exp1}${type}`)) + parseFloat(css(elem, `padding${exp1}`)) + parseFloat(css(elem, `padding${exp2}`)) + parseFloat(css(elem, `border${exp2}${type}`));
